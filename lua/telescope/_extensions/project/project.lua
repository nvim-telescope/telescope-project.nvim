...
---@return Project
function Project:new(title, path, activated, last_accessed_time)
  local obj = {
    title = title,
    path = path,
...
function Project:encode()
  ...
  if self.last_accessed_time then
    escaped_parts = { escape(sep, self.title, self.path, self.activated, self.last_accessed_time) }
  else
    escaped_parts = { escape(sep, self.title, self.path, self.activated) }
  end
...
function Project.decode(self, line)
  ...
  local title, path, activated, last_accessed_time = unescape(sep, unpack(fields))
  activated = tonumber(activated)
  if not activated then
    activated = 1
  end
  return Project:new(title, path, activated, tonumber(last_accessed_time))
end
